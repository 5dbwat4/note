...
这里需要注意：`space` 在 C 语义上是未显式初始化的全局数组，属于零初始化数据；但在当前实验实现里，判断“匿名页”不是看它是否来自 BSS，而是看 VMA 是否带 `VM_ANON`。`init_user_task()` 中只注册了两段 VMA：
...
因此 `[0x0, uapp_size)` 内的 `.text`、`.rodata`、`.data`、`.bss` 都属于同一个用户程序 VMA，不带 `VM_ANON`；只有用户栈 `[0x3ffffff000, 0x4000000000)` 是匿名 VMA。对应到你的输出，真正来自匿名页的只有 `stval = 0x3ffffffff8` 这一条栈页缺页；`0x0`、`0x11d4`、`0x2230`、`0x3000`、`0x4000` 都来自用户程序 VMA。
...
- `0x3000 D space` 来自 `build/user/uapp.map`。这个文件由 `$(NM) build/user/uapp.elf > build/user/uapp.map` 生成，表示链接器最终把全局数组 `space` 放在用户虚拟地址 `0x3000`。
- `0x2068 <__iob+0x68>` 来自 `build/user/uapp.asm`。这个文件由 `objdump -S build/user/uapp.elf` 生成。`objdump` 没有给字符串字面量单独命名，所以它用最近的已知符号 `__iob` 加偏移量来标注地址；`__iob+0x68` 只是反汇编器的地址显示方式，不表示字符串在语义上属于 `__iob` 结构。
- `0x4000 <space+0x1000>` 同样来自反汇编，它是编译器为 `&space[0x1000]` 生成的目标地址。

这些地址最终由 `user/uapp.lds` 决定。链接脚本从地址 `0` 开始放置用户程序，先放 `.text`，再放 `.data`，并把 `.rodata`、`.bss` 也并入 `.data` 输出段。`"ZJU Sys3 Lab5"` 是 C 字符串字面量，编译器会把它放入只读常量输入段 `.rodata`；但链接脚本把 `.rodata` 合并进 `.data`，所以它最终出现在用户程序镜像的数据区域中。`space` 是页对齐的全局数组，带有 `aligned(0x1000)`，因此链接器把它放到下一个 4 KiB 边界，也就是 `0x3000`。

反汇编中的 `auipc + addi` 是 RISC-V 常见的 PC 相对取地址方式。以源地址为例：
...
所以 `a1` 最终就是字符串字面量的地址 `0x2068`。目的地址同理：
...
为了让第三问的现象更直接，我对 `FORK2` 增加了两个读取探针，并把后续 `memcpy` 也改成复用同一个字符串指针：

```c
int main(void) {
  volatile const char *message = "ZJU Sys3 Lab5";
  volatile char literal_probe = message[0];
  volatile char space_probe = ((volatile char *)space)[0x1000];
  (void)literal_probe;
  (void)space_probe;

  ...

  memcpy(&space[0x1000], (const char *)message, 14);
```

这里 `volatile const char *message` 的作用是避免编译器把 `message[0]` 优化成立即数，确保真的从字符串字面量地址读内存。`((volatile char *)space)[0x1000]` 则确保在写入目的地址之前，先用 load 指令读取一次 `0x4000`。
